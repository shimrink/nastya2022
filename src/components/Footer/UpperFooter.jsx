...
const TitleWrap = styled.div`
	...
	display: flex;
	align-self: ${({ m }) =>
		m.isHugeDesk || m.isDesk ? 'center' : 'flex-start'};
	margin-top: ${({ m }) =>
		m.isTabletA ? 40 : m.isTabletP ? 96 : m.isMobile ? -100.8 : 0}px;
	overflow: hidden;
	z-index: 1;
...
const Title = styled.h2`
	...
	font-size: ${({ m }) =>
		m.isHugeDesk || m.isDesk ? 96 : m.isTabletA ? 76 : 48}px;
...
const Img = styled.img`
	...
	grid-column: ${({ m }) =>
		m.isTabletA ? '3/7' : m.isTabletP ? '1/3' : m.isMobile ? '1/3' : '5/9'};
	width: 100%;
	z-index: 2;
`
const Text = styled.p`
	grid-row: 3/4;
	grid-column: ${({ m }) =>
		m.isTabletA ? '7/11' : m.isTabletP ? '3/5' : m.isMobile ? '1/3' : '1/5'};
`
