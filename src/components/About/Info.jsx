...
const Content = styled.div`
	...
	grid-template-columns: ${({ m }) =>
		m.isTabletP || m.isMobile ? 'repeat(4, 1fr)' : 'repeat(12, 1fr)'};
	grid-column-gap: ${({ m }) => (m.isTabletP ? 40 : m.isMobile ? 0 : 24)}px;
	padding: ${({ m }) =>
		m.isHugeDesk
			? '0'
			: m.isDesk
...
const Ranks = styled.div`
	grid-row: 1/2;
	grid-column: ${({ m }) =>
		m.isTabletP ? '3/5' : m.isMobile ? '1/5' : '7/13'};
	display: flex;
...
const Landscape = styled.img`
	...
	grid-row: 4/5;
	grid-column: ${({ m }) =>
		m.isTabletP ? '3/5' : m.isMobile ? '1/4' : '7/12'};
	align-self: end;
	width: 100%;
...
const Portrait = styled.img`
	...
	grid-column: ${({ m }) => (m.isTabletP ? '1/3' : m.isMobile ? '2/5' : '2/6')};
	align-self: center;
	width: 100%;
	z-index: 1;
`
	...
	return (
		<Wrap m={media}>
			...
			<Content m={media}>
				...
				<Multi m={media} className='animItems _anim-show-opacity'>
